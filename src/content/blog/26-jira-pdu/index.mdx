...
| Delivery Model | Tool Characteristics You Need |
|---|---|
| **Waterfall / Traditional** | Gantt charts, milestone tracking, dependency mapping, resource loading, baseline vs. actual |
| **Scrum** | Sprint boards, backlog management, velocity tracking, burndown charts |
| **Kanban** | WIP limits, flow metrics, queue visualization, cycle time |
| **Hybrid** | Flexibility across both views, cross-functional reporting, portfolio-level visibility |
| **Scaled Agile (SAFe)** | Program board capability, PI objectives, ART-level reporting, dependency tracking across teams |
...
```mermaid
flowchart TD
    A([Start: Tool selection needed]) --> B{Do you have a\ndefined delivery process?}
    B -- No --> C[Define your process first.\nTool selection comes second.]
    B -- Yes --> D{What is your\ndelivery model?}
 
    D -- Waterfall --> E[Microsoft Project\nSmartsheet]
    D -- Scrum/Kanban --> F[Jira\nLinear]
    D -- Hybrid --> G[Jira + Smartsheet\nor Monday.com]
    D -- SAFe --> H[Jira Align\nTargetProcess]
    ...
    I -- No --> J[Run a time-boxed pilot\nwith real work]
    J --> I
    I -- Yes --> K{Does it integrate\nwith existing workflow?}
    K -- No --> L[Evaluate integration layer\nor reconsider tool]
    L --> K
    K -- Yes --> M([Commit and build\na configuration playbook])
```
...
*PDU Note: Writing this post counts toward PMI's Giving Back to the Profession category under Create Content. Research, tool evaluation frameworks, and hybrid delivery considerations apply across all three Talent Triangle domains: Ways of Working, Business Acumen, and Power Skills. Log time spent writing and researching separately in CCRS.*