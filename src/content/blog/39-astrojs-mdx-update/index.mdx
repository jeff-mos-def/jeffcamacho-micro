...
Dev experience. Several annoyances got fixed. Editing CSS no longer leaves stale inline styles in server-rendered HTML, killing the flash of old styles on reload. HMR now works for files in src/middleware/. Renaming an image while the dev server is running no longer crashes the build. CSS from client:only islands no longer leaks into unrelated pages when the bundler chunks modules together.

Compatibility. v6.4.1 restored the astro/jsx/rehype.js entry point so older versions of @astrojs/mdx keep working with Astro 6.x. v6.4.2 fixed a "plugins is not iterable" crash that could fire when a pre-6.0 MDX integration ran alongside something like Starlight that sets markdown plugin options. If you upgraded Astro core but not MDX, those two patches are why it didn't fall over.
